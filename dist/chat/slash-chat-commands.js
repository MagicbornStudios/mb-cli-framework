...
export function parseSlashChatLine(line) {
    const t = line.trim();
    if (!t.startsWith('/'))
        return null;
    const parts = t
    ...
    if (cmd === 'rag' || cmd === 'index') {
        ...
        if (sub === 'off')
            return { kind: 'rag', sub: 'off' };
        return { kind: 'rag', sub: 'cycle' };
    }
    return null;
}
export function formatRagModeLabel(mode) {
    if (mode === undefined)
        return 'env (SITE_CHAT_RAG)';
    if (mode === 'off')
        return 'off';
    if (mode === 'books')
        return 'books';
    if (mode === 'books_planning')
        return 'books_planning';
    return 'books_planning_repo';
}
export function cycleRagMode(stored) {
...
export function isValidChatModelId(id) {
    return MODEL_ID_RE.test(id.trim());
}
export async function fetchOpenAiChatModelIds() {
    ...
    return ids
        .filter((id) => /gpt-|o1|o3|gpt-4o|chatgpt-4o/i.test(id) || (id.includes('gpt') && !id.includes('embedding')))
...
export async function handleSlashChatLine(input) {
    ...
    if (parsed.kind === 'model') {
        ...
        if (!models.length) {
            return {
                assistantText: '└ OPENAI_API_KEY missing or models list empty. Set a model with: /model gpt-4o-mini',
            };
        }
